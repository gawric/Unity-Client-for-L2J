// shot_N_atk_v1_ta / MeshEmitter225 "Spirit"
// LIVE e_u505: UseMeshBlendMode=False, RenderTwoSided, SpinParticles,
//   SizeScale grow, FadeIn/Out, Opacity=0.2, ColorScale×7, Offset X=-5,
//   StartVelocity (10, ±20, ±20), anisotropic StartSize (0.015, 0.1, 0.1).
// No UseRotationFrom / PTRS_Actor — MeshSpin only.
// No VelocityLoss — L2FxMeshMotion ballistic.
// Blend: LIVE RenderDoc One/One Add (PTDS_Translucent).
//
// Spawn: L2FxMeshSpawnParticle_SampleLocVelSize (TLS before StartVelocity).
// Spin: L2FxMeshSpin (+ SPS sampled from same TLS after StartSpin).
// LIVE VERIFIED SpawnSoulShotSpiritCapture 2026-07-22 draws=28.
Shader "L2/Effects/shot_N_atk_v1_ta/MeshEmitter225_Spirit"
{
    Properties
    {
        ...
        _LifetimeRange ("Lifetime Min Max", Vector) = (1, 1.5, 0, 0)
        ...
        // UC StartLocationOffset=(X=-5); LocRange defaults 0 (still consumes draws).
        _StartLocationOffsetUe ("StartLocationOffset UE XYZ", Vector) = (-5, 0, 0, 0)
        ...
        _StartVelocityRangeYUc ("StartVelocity Y Min Max", Vector) = (-20, 20, 0, 0)
        _StartVelocityRangeZUc ("StartVelocity Z Min Max", Vector) = (-20, 20, 0, 0)
        _StartSizeRangeXUc ("StartSize X Min Max", Vector) = (0.015, 0.015, 0, 0)
        _StartSizeRangeYUc ("StartSize Y Min Max", Vector) = (0.1, 0.1, 0, 0)
        _StartSizeRangeZUc ("StartSize Z Min Max", Vector) = (0.1, 0.1, 0, 0)
        // TLS appRand before StartVelocity GetRand. Non-zero enables spawn stream.
        _MeshSpawnRandStateBits ("appRand TLS before StartVelocity (uint bits)", Float) = 0

        [Toggle] _UseSizeScale ("Use SizeScale", Float) = 1
        // UC SizeScale(0..3); first key @0.15 size=2 — L2Fx_SampleSizeScale lerps 1→2 before key0.
        _SizeKey0 ("SizeScale 0 Time Size", Vector) = (0.15, 2, 0, 0)
        _SizeKey1 ("SizeScale 1 Time Size", Vector) = (0.32, 2.5, 0, 0)
        _SizeKey2 ("SizeScale 2 Time Size", Vector) = (0.54, 3, 0, 0)
        _SizeKey3 ("SizeScale 3 Time Size", Vector) = (1, 3.5, 0, 0)
        _SizeKey4 ("SizeScale 4 Time Size", Vector) = (1, 3.5, 0, 0)

        [Toggle] _SpinParticles ("SpinParticles", Float) = 1
        _StartSpinYawRangeUc ("StartSpin X / Yaw", Vector) = (-0.1, 0.1, 0, 0)
        _StartSpinPitchRangeUc ("StartSpin Y / Pitch", Vector) = (-0.1, 0.1, 0, 0)
        _StartSpinRollRangeUc ("StartSpin Z / Roll", Vector) = (0, 1, 0, 0)
        _SpsYawRangeUc ("SpinsPerSecond X / Yaw", Vector) = (0.2, 0.2, 0, 0)
        _SpsPitchRangeUc ("SpinsPerSecond Y / Pitch", Vector) = (0.2, 0.2, 0, 0)
        _SpsRollRangeUc ("SpinsPerSecond Z / Roll", Vector) = (1, 2, 0, 0)
        // UC SpinCCWorCW=(Z=0) → X/Y default 1, Z=0.
        _SpinCCWorCW ("SpinCCWorCW X Y Z", Vector) = (1, 1, 0, 0)
        _StartSpinRandStateBits ("appRand TLS before StartSpin (uint bits)", Float) = 0

        _ColorMultiplier ("ColorMultiplier RGB", Vector) = (0.6, 0.6, 0.6, 0)
        _ColorScaleRepeats ("ColorScale Repeats", Float) = 7
        _ColorKey0 ("ColorScale 0", Color) = (1, 1, 1, 1)
        _ColorKey1Time ("ColorScale 1 Time", Float) = 0.5
        _ColorKey1 ("ColorScale 1", Color) = (0.70196, 0.70196, 0.70196, 1)
        _ColorKey2Time ("ColorScale 2 Time", Float) = 1
        _ColorKey2 ("ColorScale 2", Color) = (1, 1, 1, 1)
        [Toggle] _FadeIn ("FadeIn", Float) = 1
        _FadeInEndTime ("FadeIn End sec", Float) = 0.09
        [Toggle] _FadeOut ("FadeOut", Float) = 1
        _FadeOutStartTime ("FadeOut Start sec", Float) = 0.41
        // LIVE e_u505_e: Opacity@+0xD0=0.2 (UC file lists 0.3)
    ...
    SubShader
    {
        ...
        // LIVE RenderDoc: One/One Add (PTDS_Translucent). Not Brighten.
        Blend One One
        Cull Off
        ZWrite Off
        ZTest LEqual
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                {
                    // Editor preview without ParticleGroup TLS injection.
                    lifetimeSeconds = max(
                ...
                // UE FinalSize (X,Y,Z) → Unity local scale (X,Z,Y) — Wave bridge.
