// shot_atk_simple_ta / MeshEmitter1 "SparkCone"
// UC: UseRotationFrom=PTRS_Actor, SpinParticles, SizeScale grow, FadeOut only,
//   Opacity=1, ColorMul ranges (R 0.7..0.8, G 0.7..0.8, B 0.75..0.85),
//   Offset=0, StartVelocity (4,0,0), Acceleration (-10,0,0),
//   VelocityLoss (2,0,0), anisotropic StartSize (0.3, 0.18, 0.18),
//   StartSpin Z-only [0,1], SPS=0.
//
// PTRS_Actor: GameObject already carries owner TRS (TransformObjectToWorld).
// Local mesh uses L2FxPTRSActor_TransformLocalMeshUnity = S(FinalSize)*R(Spin).
// Spin slot samples stay on L2FxMeshSpin / FRangeVector (UC StartSpin/SPS);
// do NOT replace PTRS WorldMat path with MeshSpin-only rotate.
//
// Motion: L2Fx_MeshMotion_EvaluatePositionUeWithDrag (VelLoss 2 on X).
//
// Spawn: L2Fx_MeshSpawnParticle_SampleLocVelSize (TLS before StartVelocity),
// same stream as shot_N_atk_v1_ta MeshEmitter225 Spirit.
Shader "L2/Effects/shot_atk_simple_ta/MeshEmitter1_SparkCone"
{
    Properties
    {
        ...
        _ManualAge ("Manual Age sec", Range(0, 0.4)) = 0
        ...
        _StartVelocityRangeXUc ("StartVelocity X Min Max", Vector) = (4, 4, 0, 0)
        ...
        _AccelerationUc ("Acceleration UE XYZ", Vector) = (-10, 0, 0, 0)
        // UC VelocityLossRange X=2 (Y/Z=0), units 1/s.
        _VelocityLossRangeUc ("VelocityLoss per-axis 1/s", Vector) = (2, 0, 0, 0)
        _StartSizeRangeXUc ("StartSize X Min Max", Vector) = (0.3, 0.3, 0, 0)
        _StartSizeRangeYUc ("StartSize Y Min Max", Vector) = (0.18, 0.18, 0, 0)
        _StartSizeRangeZUc ("StartSize Z Min Max", Vector) = (0.18, 0.18, 0, 0)
        ...
        // UC SizeScale(0..3): (0.15,2.2) (0.31,3) (0.59,3.5) (1,4)
        _SizeKey0 ("SizeScale 0 Time Size", Vector) = (0.15, 2.2, 0, 0)
        _SizeKey1 ("SizeScale 1 Time Size", Vector) = (0.31, 3, 0, 0)
        _SizeKey2 ("SizeScale 2 Time Size", Vector) = (0.59, 3.5, 0, 0)
        _SizeKey3 ("SizeScale 3 Time Size", Vector) = (1, 4, 0, 0)
        _SizeKey4 ("SizeScale 4 Time Size", Vector) = (1, 4, 0, 0)

        [Toggle] _SpinParticles ("SpinParticles", Float) = 1
        // UC StartSpinRange Z only [0,1]; X/Y default 0.
        ...
        // Real ColorMultiplierRange (R 0.7..0.8, G 0.7..0.8, B 0.75..0.85).
        _ColorMulMin ("ColorMultiplier Min RGB", Vector) = (0.7, 0.7, 0.75, 0)
        _ColorMulMax ("ColorMultiplier Max RGB", Vector) = (0.8, 0.8, 0.85, 0)
        ...
        _FadeOutStartTime ("FadeOut Start sec", Float) = 0.0352
        ...
        Pass
        {
            ...
            Varyings vert(Attributes IN)
            {
                ...
                else
                {
                    ...
                    colorMulRgb = float3(_ColorMulMin.x, _ColorMulMin.y, _ColorMulMin.z);
                }
                ...
                // UE FinalSize (X,Y,Z) with project K — PTRS ScaleUe stays UE-ordered.
                ...
                float3 startSpinC012 = float3(0.0, 0.0, 0.0);
                float3 spinRateC012 = float3(0.0, 0.0, 0.0);
                if (_SpinParticles > 0.5)
                {
                    ...
                    // Slot c0/c1/c2 == Yaw/Pitch/Roll URU (UC X/Y/Z). PTRS swaps for R.
                    startSpinC012 = startYawPitchRollUru;
                    spinRateC012 = L2Fx_MeshSpin_VelocityYawPitchRollUruPerSecond(
                        spsUc, directionSign);
                }

                // UseRotationFrom=PTRS_Actor local mesh: S(FinalSize)*R(Spin).
                float3 localMeshOS = L2FxPTRSActor_TransformLocalMeshUnity(
                    IN.positionOS.xyz,
                    finalSizeUe,
                    spinRateC012,
                    startSpinC012,
