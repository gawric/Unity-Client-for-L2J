// Calibration ONLY: it_healing_potion_ta / m_u004_b SpriteEmitter0
...
Shader "L2/Effects/Calib/HealingPotionTaSpriteEmitter0"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            int ResolveManualFrame()
            {
                int s0 = (int)_SubdivisionStart;
                int s1 = (int)_SubdivisionEnd;
                int lo = min(s0, s1);
                int hi = max(s0, s1);
                return clamp((int)round(_ManualFrameIndex), lo, hi);
            }
            ...
            Varyings vert(Attributes IN)
            {
                ...
                int frame = ResolveManualFrame();
                OUT.uvAtlas = AtlasUV(IN.uv, frame, uSub, vSub);
                return OUT;
            }
